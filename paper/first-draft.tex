...
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage[hidelinks]{hyperref}          % hidelinks: 색 박스 제거
\usepackage{natbib}
\usepackage{caption}
...
%--- metadata --------------------------------------------------------
\title{\Large\textbf{Gene-Selection Optimisation via JTK\_Cycle Pre-Filtering:}\\
...
\author{
  ...
  {\small BCS40010 Project --- First Draft}
}
...
\emph{TimeSignature} (Braun et al., 2018) trains an elastic-net
regression over all ${\sim}$7,600 genes in a microarray panel and
recovers ${\sim}$40 predictors without any prior periodicity screen.
Subsequent methods---\emph{TimeMachine} (Huang \& Braun, 2024) and
\emph{tauFisher} (Duan \& Ngo, 2024)---insert a JTK\_Cycle
rhythmicity pre-filter before regression, reducing the candidate pool
to 20--37 genes.
We ask whether the same pre-filtering strategy can improve
TimeSignature's gene-selection efficiency, yielding a smaller
predictor set that still meets clinically acceptable accuracy
(nAUC $\geq 0.80$, MAE $< 2.5$\,h) under the original two-blood-draw
protocol.
Two experiments are conducted on four public datasets
...
A JTK-filtered pool of only 30 genes achieves
...
This pervasive regulation makes the precise estimation of an
individual's internal circadian phase---often termed the
\emph{circadian time}---a prerequisite for personalised chronotherapy,
...
precision for clinical use \citep{Moller2013}.
...
This approach achieves a normalised AUC (nAUC) of ${\sim}$0.85 and a
...
the candidate pool to 37 rhythmic genes before fitting a penalised
...
\paragraph{Within-subject normalisation.}
...
\begin{itemize}
  ...
  \item \textbf{nAUC}: normalised area under the cumulative
...
\begin{figure}[h]
  ...
  \caption{
    \textbf{Cross-study prediction accuracy for four independent datasets.}
    Original results from Braun et al. (Left) and our reproduced simulation (Right). Both show consistent and high accuracy (nAUC $\geq 0.80$, median error $< 2$\,h), successfully confirming the robustness of the within-subject normalization.
  }
...
confirming that our pipeline---within-subject normalisation, antipodal
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.72\textwidth]{figure/Exp1_tradeoff.pdf}
  \caption{
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.72\textwidth]{figure/Exp2_geneset_nAUC.pdf}
  \caption{
...
TimeMachine-37 (nAUC\,${=}\,0.844$) falls \emph{above} the
Experiment~1 curve at $K = 37$ (nAUC\,${=}\,0.825$), indicating
that TimeMachine's gene-selection procedure---core-clock genes
prioritised, then JTK-ranked additions---outperforms a pure top-37
JTK ranking by $+0.019$ nAUC.
...
well-characterised high-amplitude 24-h expression rhythms in
...
than an artefact of the training cohort.
...
be pre-filtered, holding the regression architecture, normalisation
strategy, and evaluation protocol constant.
This design isolates the effect of gene pre-filtering from
confounding changes in normalisation (e.g., TimeMachine's
...
than the single-sample normalisation---accounts for most of the gain
...
more conservative estimate of generalisation accuracy.
...
the Archer dataset presents a harder generalisation challenge whose
...
single-draw normalisation strategies---such as TimeMachine's
